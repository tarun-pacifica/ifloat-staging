class ObjectVerifier
  ...
  def verify_same_image_means_same_group
    ...
    @primary_images_by_ref.each do |product_ref, asset_ref|
      ...
      @errors << error_for_row("has the same primary image as #{ident(first_ref)} but #{problem}", ref.row_md5s.first)
    end
  end
  ...
  def verify_unique_titles
    ...
    @text_values_by_prop_name_by_ref.each do |ref, tvs_by_pn|
      (tvs_by_pn["auto:title"] || []).each do |tv|
        ...
        if existing_ref.nil? then first_refs_by_value[value] = ref
        else @errors << error_for_row("has the same #{heading} title as #{ident(existing_ref)}: #{value}", ref.row_md5s.first)
        end
      end
    end
  end
  ...
  def verify_well_differentiated_siblings
    ...
    values_by_ref_by_group.each do |group, values_by_ref|
      ...
      values_by_ref.values.transpose.each_with_index do |diff_column, i|
        ...
        @errors << error_for_row("differentiating values from property set #{i + 1} are a mixture of values and blanks: #{diff_column.inspect} (group: #{friendly_group})", values_by_ref.keys.first.row_md5s.first)
      end
      ...
      values_by_ref.to_a.combination(2) do |a, b|
        ...
        @errors << error_for_row("has the same differentiating values #{values1.inspect} as #{ident(ref1)} (group: #{friendly_group})", ref2.row_md5s.first)
      end
    end
  end
  ...
  def ident(prod_ref)
    ...
    "#{company[:reference]} / #{product[:reference]} (#{@csvs.location(prod_ref.row_md5s.first)})"
  end
end
